...
def channel_shuffle(x, groups):
    ...
    assert (num_channels % 4 == 0 and groups == 2)
    ...
    channels_split = torch.split(x, channels_per_group, dim=1)
    channels_concat = torch.cat([channels_split[0],channels_split[2],channels_split[1],channels_split[3]],dim=1)
    #xx = x.clone()#torch.Tensor(batchsize, num_channels, height, width).cuda().copy_(x)
    #xx[:, channels_per_group * 1:channels_per_group * 2, :, :] = x[:, channels_per_group * 2:channels_per_group * 3, :, :]
    #xx[:, channels_per_group * 2:channels_per_group * 3, :, :] = x[:, channels_per_group * 1:channels_per_group * 2, :, :]

    return channels_concat
...
class BasicUnit(nn.Module):
    ...
    def forward(self, x):
        x_split = torch.split(x, self.left_part, dim=1)
        left = x_split[0]
        right = x_split[1]
        ...
        tmp_result = torch.cat((left, out), 1)
        final_result = channel_shuffle(tmp_result, self.groups)
...
class swap_net_v2(nn.Module):
    """ShuffleNetV2 implementation.
    """

    def __init__(self, scale=1.0, in_channels=3, n=[3, 8, 3], c_tag=0.5, num_classes=1, activation=nn.ReLU,
        ...
        super(swap_net_v2, self).__init__()
        ...
        self.num_of_channels = {0.35: [24, 48, 64, 128, 192], 0.5: [24, 48, 96, 192, 1024], 0.75: [24, 80, 160, 256, 1024], 1: [24, 116, 232, 464, 1024],
                                1.5: [24, 176, 352, 704, 1024], 2: [24, 244, 488, 976, 2048]}
        self.c = [_make_divisible(chan, groups) for chan in self.num_of_channels[scale]]
        self.n = n  # TODO: should be [3,7,3]
        self.conv1 = nn.Conv2d(in_channels, self.c[0], kernel_size=3, bias=False, stride=2, padding=1)
        self.bn1 = nn.BatchNorm2d(self.c[0])
        self.maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
		#self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2)
        #self.maxpool = nn.Conv2d(self.c[0], self.c[0], kernel_size=3, stride=2, groups=self.c[0], bias=False)
        #self.bn2 = nn.BatchNorm2d(self.c[0])
...
if __name__ == "__main__":
    """Testing
    """
    model1 = swap_net_v2(scale=0.35, n=[2, 4, 2]) #for 96x96, 0.14ms on CV22
    # model2 = swap_net_v2(scale=0.75, n=[3, 8, 3]) #for detection
    model2 = swap_net_v2(scale=1.00, n=[3, 8, 3]) #shufflenet
    x2 = torch.randn(8, 3, 224, 224, )
    x2=nn.functional.sigmoid(x2)
    result=model2(x2)
    print(model2(x2))
    # model2 = swap_net_v2(scale=0.5, in_channels=3, c_tag=0.5, num_classes=1000, activation=nn.ReLU,
    #                       SE=False, residual=False)
    # print(model2)
    # model3 = swap_net_v2(in_channels=2, num_classes=10)
    # print(model3)
    # x = torch.randn(1, 2, 224, 224)
    # print(model3(x))
    # model4 = swap_net_v2( num_classes=10, groups=3, c_tag=0.2)
    # print(model4)
    # model4_size = 769
    # x2 = torch.randn(1, 3, model4_size, model4_size, )
    # print(model4(x2))
    # model5 = swap_net_v2(scale=2.0,num_classes=10, SE=True, residual=True)
